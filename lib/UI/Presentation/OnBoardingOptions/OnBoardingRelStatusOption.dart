...
class OnBoardingRelStatusOption extends StatefulWidget {
  OnBoardingRelStatusOption({
    Key key,
    this.isSelected,
    this.isSelectable,
    this.relationshipstatuse,
    this.onboarding,
    this.onboardingDao,
    this.onItemClick,
  ...
  @override
  _OnBoardingRelStatusOptionState createState() =>
      new _OnBoardingRelStatusOptionState();
}

class _OnBoardingRelStatusOptionState extends State<OnBoardingRelStatusOption>
    with AfterLayoutMixin<OnBoardingRelStatusOption> {
  @override
  void initState() {
    super.initState();
  }
            ...
            children: <Widget>[
              ...
              Expanded(
                child: InkWell(
                  ...
                  onTap: () {
                    print('OnBoardingRelStatusOption AM TAPPED');
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      boxShadow: [
                        BoxShadow(
                            blurRadius: .5,
                            spreadRadius: 1.0,
                            /*color: widget.relationshipstatuse.id ==
                                    widget.onboarding.relationshipstatus
                                ? Color(0xFFFFFFFF)
                    ...
                    child: Row(
                      ...
                      children: <Widget>[
                        Text(
                          widget.relationshipstatuse.name,
                          style: widget.relationshipstatuse.id ==
                                  widget.onboarding.relationshipstatus
                              ? datingAppThemeChanger.selectedThemeData
  ...
  updateOnBoarding() async {
    if (widget.isSelected) {
      ...
      bool updated = await widget.onboardingDao.updateOnboarding(widget
          .onboarding
          .toCompanion(false)
          .copyWith(relationshipstatus: Value(null)));
      if (updated) {
      ...
      widget.onBoardingClickableItemBLoC.onboarding_itemclicked_event_sink
          .add(OneItemClickedEvent(widget.relationshipstatuse.id));
    } else {
      ...
      bool updated = await widget.onboardingDao.updateOnboarding(widget
          .onboarding
          .toCompanion(false)
          .copyWith(relationshipstatus: Value(widget.relationshipstatuse.id)));
      if (updated) {
        Onboarding updatedOnboarding =
            await widget.onboardingDao.getOnboardingById(widget.onboarding.id);
        /*setState(() {
  ...
  refreshOnBoarding() async {
    ...
    setState(() {
      widget.onboarding = updatedOnboarding;
      if (widget.relationshipstatuse.id ==
          widget.onboarding.relationshipstatus) {
        widget.isSelected = true;
      } else {
        widget.isSelected = false;
