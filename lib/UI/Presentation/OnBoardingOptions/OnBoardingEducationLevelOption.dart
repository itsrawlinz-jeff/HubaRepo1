...
class OnBoardingEducationLevelOption extends StatefulWidget {
  OnBoardingEducationLevelOption({
    Key key,
    this.isSelected,
    this.isSelectable,
    this.educationlevel,
  ...
  @override
  _OnBoardingEducationLevelOptionState createState() =>
      new _OnBoardingEducationLevelOptionState();
}

class _OnBoardingEducationLevelOptionState
    extends State<OnBoardingEducationLevelOption>
    with AfterLayoutMixin<OnBoardingEducationLevelOption> {
  @override
  void initState() {
    super.initState();
  }
                    ...
                    decoration: BoxDecoration(
                      boxShadow: [
                        BoxShadow(
                            blurRadius: .5,
                            spreadRadius: 1.0,
                            /*color: widget.educationlevel.id ==
                                widget.onboarding.education_level
                            ? Color(0xFFFFFFFF)
                    ...
                    child: Row(
                      ...
                      children: <Widget>[
                        Text(
                          ...
                          style: widget.educationlevel.id ==
                                  widget.onboarding.education_level
                              ? datingAppThemeChanger.selectedThemeData
  ...
  updateOnBoarding() async {
    if (widget.isSelected) {
      ...
      bool updated = await widget.onboardingDao.updateOnboarding(widget
          .onboarding
          .toCompanion(false)
          .copyWith(education_level: Value(null)));
      /*if (updated) {
      ...
      bool updated = await widget.onboardingDao.updateOnboarding(widget
          .onboarding
          .toCompanion(false)
          .copyWith(education_level: Value(widget.educationlevel.id)));
      /*if (updated) {
        Onboarding updatedOnboarding =
            await widget.onboardingDao.getOnboardingById(widget.onboarding.id);
        setState(() {
  ...
  refreshOnBoarding() async {
    ...
    setState(() {
      widget.onboarding = updatedOnboarding;
      if (widget.educationlevel.id == widget.onboarding.education_level) {
        widget.isSelected = true;
      } else {
        widget.isSelected = false;
      }
