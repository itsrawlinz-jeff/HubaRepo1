...
class OnBoardingWantChildrenOption extends StatefulWidget {
  OnBoardingWantChildrenOption({
    Key key,
    this.isSelected,
    this.isSelectable,
    this.wantchildrenstatuse,
    this.onboarding,
    this.onboardingDao,
    this.onItemClick,
  ...
  @override
  _OnBoardingWantChildrenOptionState createState() =>
      new _OnBoardingWantChildrenOptionState();
}

class _OnBoardingWantChildrenOptionState
    extends State<OnBoardingWantChildrenOption>
    with AfterLayoutMixin<OnBoardingWantChildrenOption> {
  @override
  void initState() {
    super.initState();
  }
                ...
                child: InkWell(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      boxShadow: [
                        BoxShadow(
                            blurRadius: .5,
                            spreadRadius: 1.0,
                            /*color: widget.wantchildrenstatuse.id ==
                                widget.onboarding.want_children
                            ? Color(0xFFFFFFFF)
                    ...
                    child: Row(
                      ...
                      children: <Widget>[
                        Text(
                          widget.wantchildrenstatuse.name,
                          style: widget.wantchildrenstatuse.id ==
                              widget.onboarding.want_children
                              ? datingAppThemeChanger.selectedThemeData
  ...
  updateOnBoarding() async {
    if (widget.isSelected) {
      ...
      bool updated = await widget.onboardingDao.updateOnboarding(widget
          .onboarding
          .toCompanion(false)
          .copyWith(want_children: Value(null)));
      if (updated) {
      ...
      widget.onBoardingClickableItemBLoC.onboarding_itemclicked_event_sink
          .add(OneItemClickedEvent(widget.wantchildrenstatuse.id));
    } else {
      ...
      bool updated = await widget.onboardingDao.updateOnboarding(widget
          .onboarding
          .toCompanion(false)
          .copyWith(want_children: Value(widget.wantchildrenstatuse.id)));
      if (updated) {
        Onboarding updatedOnboarding =
            await widget.onboardingDao.getOnboardingById(widget.onboarding.id);
        /*setState(() {
  ...
  refreshOnBoarding() async {
    ...
    setState(() {
      widget.onboarding = updatedOnboarding;
      if (widget.wantchildrenstatuse.id == widget.onboarding.want_children) {
        widget.isSelected = true;
      } else {
        widget.isSelected = false;
      }
