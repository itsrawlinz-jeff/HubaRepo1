...
class OnBoardingOftenDrinkOption extends StatefulWidget {
  OnBoardingOftenDrinkOption({
    Key key,
    this.isSelected,
    this.isSelectable,
    this.drinkstatuse,
  ...
  @override
  _OnBoardingOftenDrinkOptionState createState() =>
      new _OnBoardingOftenDrinkOptionState();
}

class _OnBoardingOftenDrinkOptionState extends State<OnBoardingOftenDrinkOption>
    with AfterLayoutMixin<OnBoardingOftenDrinkOption> {
  @override
  void initState() {
    super.initState();
  }
                      ...
                      boxShadow: [
                        BoxShadow(
                            ...
                            /*color: widget.drinkstatuse.id ==
                                widget.onboarding.often_you_drink
                            ? Color(0xFFFFFFFF)
  ...
  updateOnBoarding() async {
    if (widget.isSelected) {
      ...
      bool updated = await widget.onboardingDao.updateOnboarding(widget
          .onboarding
          .toCompanion(false)
          .copyWith(often_you_drink: Value(null)));
      /*if (updated) {
      ...
      bool updated = await widget.onboardingDao.updateOnboarding(widget
          .onboarding
          .toCompanion(false)
          .copyWith(often_you_drink: Value(widget.drinkstatuse.id)));
      /*if (updated) {
        Onboarding updatedOnboarding =
            await widget.onboardingDao.getOnboardingById(widget.onboarding.id);
        setState(() {
  ...
  refreshOnBoarding() async {
    ...
    setState(() {
      widget.onboarding = updatedOnboarding;
      if (widget.drinkstatuse.id == widget.onboarding.often_you_drink) {
        widget.isSelected = true;
      } else {
        widget.isSelected = false;
      }
