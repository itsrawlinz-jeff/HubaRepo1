...
import 'package:dating_app/utils/data_converters.dart';
...
class PhoneNoAmountDialog extends StatefulWidget {
  ...
  @override
  _PhoneNoAmountDialogState createState() => new _PhoneNoAmountDialogState();
}

class _PhoneNoAmountDialogState extends State<PhoneNoAmountDialog>
    with TickerProviderStateMixin, AfterLayoutMixin<PhoneNoAmountDialog> {
  List<String> str = [];
  AnimationController animationController;
  ...
  TextEditingController amount_txt_Controller = TextEditingController();
  ...
  NavigationDataBLoC val_amount_NavigationDataBLoC = NavigationDataBLoC();

  NavigationDataBLoC loader_NavigationDataBLoC = NavigationDataBLoC();

  NavigationDataBLoC wd_name_Container_NavigationDataBLoC =
      NavigationDataBLoC();
  NavigationDataBLoC wd_amount_Container_NavigationDataBLoC =
      NavigationDataBLoC();
  ...
  void afterFirstLayout(BuildContext context) {
    String TAG = 'PhoneNoAmountDialog afterFirstLayout:';
  ...
  refresh_wd_amount_Container_NavigationDataBLoC(bool isValid, String msg) {
    NavigationData navigationData = NavigationData();
    navigationData.isValid = isValid;
    navigationData.message = msg;
    wd_amount_Container_NavigationDataBLoC.dailySubTaskimages_event_sink
        .add(NavigationDataAddedEvent(navigationData));
  }
    ...
    return new Padding(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          Padding(
            padding: EdgeInsets.only(left: 4, right: 16, top: 3, bottom: 3),
            child: Text(
              'Amount',
              style: datingAppThemeChanger
                  .selectedThemeData.txt_stl_whitegrey_14_Med,
            ...
            child: Row(
              ...
              children: <Widget>[
                Expanded(
                    child: Column(
                  children: <Widget>[
                    amount_StreamBuilder(datingAppThemeChanger),
                    StreamBuilder(
                      stream:
                          wd_amount_Container_NavigationDataBLoC.stream_counter,
                      builder: (context, snapshot) {
                        if (snapshot.hasError) {
                          return invisibleWidget();
                        }
  ...
  Widget amount_StreamBuilder(DatingAppThemeChanger datingAppThemeChanger) {
    return StreamBuilder(
      stream: val_amount_NavigationDataBLoC.stream_counter,
      builder: (context, snapshot) {
        return Container(
          height: chooseContainerWidth(snapshot),
          decoration: BoxDecoration(
          ...
          child: Padding(
            padding: EdgeInsets.only(left: 16, right: 16, top: 0, bottom: 0),
            child: amount_TextFormField(datingAppThemeChanger),
          ),
        );
      },
    );
  ...
  Widget first_name_TextFormField(DatingAppThemeChanger datingAppThemeChanger) {
    return TextFormField(
      ...
      onFieldSubmitted: (String val) {
        ...
        amount_FocusNode.requestFocus();
      },
    );
  }

  Widget amount_TextFormField(DatingAppThemeChanger datingAppThemeChanger) {
    return TextFormField(
      keyboardType: TextInputType.numberWithOptions(decimal: true),
      style: datingAppThemeChanger.selectedThemeData.txt_stl_f14w500_Med,
      cursorColor: datingAppThemeChanger.selectedThemeData.cl_grey,
      ...
      controller: amount_txt_Controller,
  ...
  illnessNameChanged() {
    widget.phone_no.phone_no = first_name_txt_Controller.text;
    validate_Phone_No();
  }

  amountChanged() {
    widget.phone_no.amount = intFromString(amount_txt_Controller.text);
    validateAmount();
  }
  ...
  bool validate_Phone_No() {
    widget.phone_no.phone_no = first_name_txt_Controller.text;
    if (isStringValid(widget.phone_no.phone_no)) {
      RegExp regex = new RegExp(get_Phone_Pattern());
      if (!regex.hasMatch(widget.phone_no.phone_no)) {
  ...
  bool validateAmount() {
    widget.phone_no.amount = intFromString(amount_txt_Controller.text);
    if (widget.phone_no.amount != null && widget.phone_no.amount != 0) {
      refresh_wd_amount_Container_NavigationDataBLoC(true, null);
      return true;
    } else {
      refresh_wd_amount_Container_NavigationDataBLoC(false, 'amount is invalid');
      return false;
    }
  }
  ...
  bool isDataValid() {
    ...
    bool is_validateAmount = validateAmount();
