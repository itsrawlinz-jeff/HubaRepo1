...
class AddNewIllnessDialog extends StatefulWidget {
  AnimationController animationController;
  BuildContext snackbarBuildContext;
  NavigationDataBLoC illnesssAddedNavigationDataBLoC;
  Illness illness;
  AddNewIllnessDialog({
    Key key,
    this.animationController,
    this.snackbarBuildContext,
    this.illnesssAddedNavigationDataBLoC,
    this.illness,
  }) : super(key: key);
  @override
  _AddNewIllnessDialogState createState() => new _AddNewIllnessDialogState();
}

class _AddNewIllnessDialogState extends State<AddNewIllnessDialog>
    with TickerProviderStateMixin, AfterLayoutMixin<AddNewIllnessDialog> {
  List<String> str = [];
  AnimationController animationController;
  ...
  void afterFirstLayout(BuildContext context) {
    String TAG = 'AddNewIllnessDialog afterFirstLayout:';
  ...
  setUpListeners() {
    illnessSavedDailySubTaskImagesBLoC.stream_counter.listen((value) {
      NavigationData navigationData = value;
      if (navigationData != null && navigationData.illness != null) {
        widget.illness = navigationData.illness;
        print('MY SAVED ID IS Listeners==${widget.illness.id}');
        print('MY SAVED onlineid IS Listeners==${widget.illness.onlineid}');
        if (isIntValid(widget.illness.onlineid)) {
  ...
  refreshonlineusersDailySubTaskImagesBLoCListeners(
    ...
    widget.illnesssAddedNavigationDataBLoC.dailySubTaskimages_event_sink
  ...
  illnessNameChanged() async {
    if (isStringValid(first_name_txt_Controller.text)) {
      List<Illness> illnessExisting =
          await illnessDao.getIllnesssByName(first_name_txt_Controller.text);
      if (illnessExisting.length > 0) {
        refresh_wd_name_Container_NavigationDataBLoC(
            false, 'Illness already exists');
        IllnesssCompanion illnesssCompanionv =
            widget.illness.toCompanion(false).copyWith(
                  name: flutterMoor.Value(null),
                  issettobeupdated: flutterMoor.Value(true),
                );
        widget.illness = database.illnesss.mapFromCompanion(illnesssCompanionv);
      } else {
        refresh_wd_name_Container_NavigationDataBLoC(true, null);
        widget.illness = widget.illness.copyWith(
      ...
      refresh_wd_name_Container_NavigationDataBLoC(
          false, 'Illness name is required');
    }
  }

  Future<bool> illnessAlreadyExists(String illnessname) async {
    List<Illness> illnessExisting = await illnessDao.getIllnesssByName(illnessname);
    if (illnessExisting.length > 0) {
      return true;
    }
    return false;
  ...
  saveData(BuildContext context) async {
    String TAG = 'saveData';
    print(TAG);
    print(widget.illness.name);
    if (_key.currentState.validate()) {
      _key.currentState.save();

      if (!isStringValid(widget.illness.name)) {
      ...
      bool is_illnessAlreadyExists =
          await illnessAlreadyExists(widget.illness.name);
      if (is_illnessAlreadyExists) {
        refresh_wd_name_Container_NavigationDataBLoC(
            false, 'Illness already exists');
      ...
      bool isput_post_illness = await put_post_illness(
        context,
        widget.snackbarBuildContext,
        widget.illness,
        null,
        illnessSavedDailySubTaskImagesBLoC,
      );
      if (isput_post_illness) {
        print('MY SAVED ID IS==${widget.illness.id}');
        print('MY SAVED onlineId IS==${widget.illness.onlineid}');
        if (isIntValid(widget.illness.onlineid)) {
          refreshonlineusersDailySubTaskImagesBLoCListeners(
              true, widget.illness);
        }
