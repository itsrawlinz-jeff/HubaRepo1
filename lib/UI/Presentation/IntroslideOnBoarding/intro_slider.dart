...
class IntroSlider extends StatefulWidget {
  ...
  IntroSliderState createState() {
    return new IntroSliderState(
      ...
      pagerIndicatorViewModel: this.pagerIndicatorViewModel,
...
class IntroSliderState extends State<IntroSlider>
  ...
  void setupButtonDefaultValues() {
    ...
    //PagerIndicatorViewModel
    if (pagerIndicatorViewModel == null) {
      ...
      body: DefaultTabController(
        ...
        child: Stack(
          children: <Widget>[
            ...
            PageDragger(
              pageLength: pagerIndicatorViewModel.pages.length - 1,
              currentIndex: pagerIndicatorViewModel.activeIndex,
              canDragLeftToRight: pagerIndicatorViewModel.activeIndex > 0,
              canDragRightToLeft: pagerIndicatorViewModel.activeIndex <
                  pagerIndicatorViewModel.pages.length - 1,
              slideUpdateStream: pagerIndicatorViewModel.slideUpdateStream,
            ),
          ],
        ),
      ),
  ...
  Widget renderBottom() {
    //TRANSFORM
    final bubbleWidth = 20.0;
    var baseTranslation =
        ((slides.length * bubbleWidth) / 2) - (bubbleWidth / 2);
    var translation =
        baseTranslation - (pagerIndicatorViewModel.activeIndex * bubbleWidth);
  ...
  List<Widget> renderListDots() {
    ...
    for (int i = 0; i < slides.length; i++) {
      dots.add(renderDot(sizeDots[i], colorDot, opacityDots[i],
          i == currentTabIndex ? true : false));
    }
    return dots;
  }

  Widget renderDoti(double radius, Color color, double opacity) {
    return Opacity(
      child: Container(
        decoration: BoxDecoration(
  ...
  Widget renderDot(double radius, Color color, double opacity, bool isActive) {
